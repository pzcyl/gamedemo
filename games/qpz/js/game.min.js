...
shareButton.onMouseDown = function(event) {
	event.preventDefault();
	//play68_submitScore(myScore,myText);
	// dp_share();
};
...
function encourage(timer) {
	if(timer < 8){
		...
		return "“有一种把它调平的感觉在心中发芽。”"
	}else if (timer < 20){
		return "“呼，好像刚刚自学完成力学平衡！”"
	}else if (timer < 30){
		return "“我怀疑我是不是天平座的！”"
	}else if (timer < 40){
		return "“其实，我是天平投胎来的！”"
...
function fillRankText(percentage) {
	...
	myScore = timer.toFixed(2);
	myText = encourage(timer);
	//updateShare(myScore,myText);
	//Play68.setRankingScoreDesc(timer.toFixed(2)*1000);
